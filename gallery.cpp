...
void gallery::addPainting(int index, const shared_ptr<artwork_instance> &work)
{
	...
	works_displayed[index] = shared_ptr<artwork_instance>(new artwork_instance(*work));

	//center on eye level, unless painting is within .5 of floor
	float eye_level = 1.65f;
	float y_offset = 0.0f;
	float min_distance_from_floor = .5f;
	if ((works_displayed[index]->getHeight() * .0067f) + min_distance_from_floor > eye_level)
		y_offset = (works_displayed[index]->getHeight() / 200.0f) + min_distance_from_floor;

	else y_offset = eye_level - (works_displayed[index]->getHeight() / 600.0f);
